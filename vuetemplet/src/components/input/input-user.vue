...
<script>
    export default{
        components:{
            gInput:require('xxx/xxx/input-user.vue')
        }
    }
</script>
@参数说明:
...
-->
<template>
    <g-input :opt.sync="userOpt" :value.sync="value" :ischeck.sync="ischeck">
    ...
    export default {
        ...
        data(){
            return {
                userOpt: {
                    ...
                    placeholder: '',
                    checkType: ['notEmpty', 'isLegitimateStr', 'hasSpace'],
                    minLength: 5,
                    maxLength: 50,
                    name: this.name
                }
        ...
        ready(){
            ...
            lan.getLan(window.SystemGlobe.component.componentI18n).then(function (lanObj) {
                that.userOpt.placeholder = lanObj.input.user;
            });
        }
    }
</script>