...
@创建时间:2016-11-11
@版本:v1.0.0
@描述:整型数据输入控件，基于input插件
@使用方法:
<template>
    <g-input-int name='key' :label="i18n.key"></g-input-int>
</template>
<script>
    export default{
        components:{
            gInputInt:require('xxx/xxx/input-integer.vue')
        }
    }
</script>
@参数说明:
...
-->
<template>
    <g-input :opt.sync="intOpt" :value.sync="value" :ischeck.sync="ischeck">
    ...
    export default {
        ...
        data(){
            return {
                intOpt: {
                    ...
                    placeholder: '',
                    checkType: ['isLegitimateStr', 'hasSpace', 'isNumber'],
                    minLength: 1,
                    maxLength: 7,
                    name: this.name
                }
        ...
        ready(){
            ...
            lan.getLan(window.SystemGlobe.component.componentI18n).then(function(lanObj){
                that.intOpt.placeholder = lanObj.input.int;
            });
        }
    }
</script>