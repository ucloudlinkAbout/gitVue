...
<script>
    export default{
        components:{
            gInput:require('xxx/xxx/input-password.vue')
        }
    }
</script>
@参数说明:
...
-->
<template>
    <g-input :opt.sync="passwordOpt" :value.sync="value" :ischeck.sync="ischeck">
    ...
    export default {
        ...
        ready(){
            ...
            lan.getLan(window.SystemGlobe.component.componentI18n).then(function (lanObj) {
                that.passwordOpt.placeholder = lanObj.input.password;
            });
        }
    }
</script>